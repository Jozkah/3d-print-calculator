...
// UV substrate catalogue — the blanks and sheet stock we print on. Deliberately
// separate from laser_materials: the two stock lists barely overlap in practice.
...
function MaterialForm({ form, onChange }: { form: FormState; onChange: (next: FormState) => void }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <div>
        <Label>Name</Label>
        <Input value={form.name} placeholder="Acrylic blank 90×90" onChange={(e) => onChange({ ...form, name: e.target.value })} className="bg-card" />
      </div>
      <div>
        <Label>Priced</Label>
      ...
      <div>
        ...
        <Input value={form.color} placeholder="#d8d8d8" onChange={(e) => onChange({ ...form, color: e.target.value })} className="bg-card" />
      </div>
    </div>
  )
}

export function UvMaterialsList({ materials }: { materials: UvMaterial[] }) {
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from("uv_materials").insert([{ ...formToRow(addForm), created_at: new Date().toISOString() }])
    if (error) {
  ...
  const handleSaveEdit = async () => {
    if (!editingId) return
    const supabase = createClient()
    const { error } = await supabase.from("uv_materials").update(formToRow(editForm)).eq("id", editingId)
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" })
      return
    }
  ...
  const handleDelete = async (m: UvMaterial) => {
    const supabase = createClient()
    const { error } = await supabase.from("uv_materials").delete().eq("id", m.id)
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" })
      return
    }
    ...
    <div className="space-y-4">
      <Card className="p-5 sm:p-6 shadow-sm">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold tracking-tight text-foreground">UV Materials</h2>
      ...
      {materials.length === 0 && !showAdd && (
        <Card className="p-8 text-center text-sm text-muted-foreground shadow-sm">
          No UV substrates yet. Add the blanks and sheet stock you print on — each priced the way you buy it.
        </Card>
      )}
