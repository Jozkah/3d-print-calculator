...
export function ExcelCalculator({
  ...
  // ADDED HANDLER TO PREVENT NON-NUMERIC INPUT (EXCEPT DECIMAL AND MINUS)
  const handleNumberKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Allow: backspace, delete, tab, escape, enter, decimal point, and minus
    if (
      e.key === "Backspace" ||
      e.key === "Delete" ||
      e.key === "Tab" ||
  ...
  // Utility functions to update state arrays
  const updatePrintedPart = (index: number, field: keyof PrintedPart, value: any) => {
    setPrintedParts((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }
  ...
  const updateDriedBatch = (index: number, field: keyof DriedBatch, value: any) => {
    setDriedBatches((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }

  const addDriedBatch = () => {
    setDriedBatches((prev) => [...prev, { id: Date.now().toString(), material: "", drying_time_hr: 0, cost: 0 }])
  }

  const updateMaterial = (index: number, field: keyof Material, value: any) => {
    setMaterials((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }

  const addMaterial = () => {
    setMaterials((prev) => [...prev, { id: Date.now().toString(), name: "", quantity: 0, unit_cost: 0 }])
  }

  const updateLabor = (index: number, field: keyof Labor, value: any) => {
    setLabor((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }

  const addLaborItem = () => {
    setLabor((prev) => [...prev, { id: Date.now().toString(), action: "", hours: 0, hourly_cost: 0 }])
  }

  const updatePackaging = (index: number, field: keyof Packaging, value: any) => {
    setPackaging((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }

  const addPackagingItem = () => {
    setPackaging((prev) => [...prev, { id: Date.now().toString(), name: "", quantity: 0, unit_cost: 0 }])
  }
  ...
  const handleSaveQuote = async () => {
    ...
    try {
      ...
      console.log("[v0] Quote saved successfully, showing toast notification")
      toast({
        title: "Success",
  ...
  return (
    <div className="space-y-6 p-6 bg-white rounded-lg shadow-sm border border-blue-100">
      {/* Quote Details */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div
              className={`p-4 rounded-lg border-2 text-center cursor-pointer hover:shadow-lg transition-shadow ${
                selectedMargin === 30 ? "bg-blue-100 border-blue-500 ring-2 ring-blue-500" : "bg-white border-blue-300"
              }`}
            ...
            <div
              className={`p-4 rounded-lg border-2 text-center cursor-pointer hover:shadow-lg transition-shadow ${
                selectedMargin === 40 ? "bg-blue-100 border-blue-500 ring-2 ring-blue-500" : "bg-white border-blue-300"
              }`}
            ...
            <div
              className={`p-4 rounded-lg border-2 text-center cursor-pointer hover:shadow-lg transition-shadow ${
                selectedMargin === 50 ? "bg-blue-100 border-blue-500 ring-2 ring-blue-500" : "bg-white border-blue-300"
              }`}
            ...
            <div
              className={`p-4 rounded-lg border-2 text-center cursor-pointer hover:shadow-lg transition-shadow ${
                selectedMargin === 60 ? "bg-blue-100 border-blue-500 ring-2 ring-blue-500" : "bg-white border-blue-300"
              }`}
